...
const {product} = defineProps<{product: IProductCard}>()
</script>

<template>
    <div class="rounded-xl bg-white relative border border-gray-100">
        ...
        <div class="m-5 mt-4">
            ...
            <p class="mt-3  line-clamp-2">{{  product.description }}</p>
            <div class="flex justify-between items-center mt-4">
                <button class="text-white bg-primary py-[13px] px-8 rounded-md">Выбрать</button>
                <span class="text-primary font-semibold text-lg">от {{ product.price }}$</span>
            </div>
        </div>
    </div>
</template>
